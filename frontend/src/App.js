...
// ── Staff-only route guard — blocks STAFF from accessing restricted pages ──
function StaffBlock() {
  return <div style={{ padding: 20, textAlign: 'center' }}>Access denied. Redirecting...</div>;
}
...
function ConditionalChatWidget() {
  ...
  // Don't show chat widget on admin routes or when admin is logged in
  const isAdminRoute = location.pathname.startsWith('/admin') || 
                       location.pathname.startsWith('/staff') || 
                       location.pathname.startsWith('/client');
  
  // Show chat widget only for public users
  const showChatWidget = !isAdminRoute;
  
  return showChatWidget ? <ChatWidget isAdmin={false} /> : null;
}

// Redirect component for root path based on authentication
function RedirectToAppropriatePage() {
  ...
  // If not authenticated, redirect to store
          ...
          <CartProvider>
            ...
            <Routes>
              ...
              <Route path="/admin/super" element={<AdminPrivateRoute><SuperAdminLayout /></AdminPrivateRoute>}>
                ...
                {/* Add admin chat route to super admin */}
              ...
              <Route path="/admin/internet" element={<DeptLayout slug="internet" title="Internet Distribution" />}>
                ...
                {/* Add admin chat route to department */}
                <Route path="chat"         element={<MessagesPage />} />
              </Route>
              ...
              <Route path="/admin/playstation" element={<DeptLayout slug="playstation" title="PlayStation Arena" />}>
                ...
                <Route path="sessions"     element={<Soon label="Sessions" />} />
                <Route path="transactions" element={<Soon label="Transactions" />} />
                <Route path="crm"          element={<Soon label="CRM" />} />
                <Route path="billing"      element={<Soon label="Billing" />} />
