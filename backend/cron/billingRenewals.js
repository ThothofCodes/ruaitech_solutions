    ...
    for (const isp of dueISP) {
      ...
      if (!exists) {
        await Invoice.create({
          clientId:       isp._id, departmentSlug: 'internet',
          lineItems:[{ description:`${isp.packageName} Renewal`, qty:1, unitPrice:isp.monthlyFee, total:isp.monthlyFee }],
          totalAmount: isp.monthlyFee, balance: isp.monthlyFee, status:'DRAFT',
          dueDate: isp.cycleEnd, invoiceId:`RTS-INT-${Date.now()}`,
        });
