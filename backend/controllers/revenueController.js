...
exports.getRevenue = async (req, res, next) => {
  try {
    ...
    const [entries, total] = await Promise.all([
      Revenue.find(query).populate('createdBy','name').sort('-date').skip((page-1)*limit).limit(limit),
...
exports.createRevenue = async (req, res, next) => {
  try {
    const { type, category, description, amount, date, paymentMethod, reference } = req.body;
