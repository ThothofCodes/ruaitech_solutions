...
exports.requestEmail = async (req, res, next) => {
  try {
    ...
    if (user.departmentSlug !== req.user.departmentSlug && req.user.role !== 'SUPER_ADMIN')
      return res.status(403).json({ message: 'Cannot request email for staff in another department' });
...
exports.suspend = async (req, res, next) => {
  try {
    const record = await CompanyEmail.findByIdAndUpdate(req.params.emailId,
...
exports.revoke = async (req, res, next) => {
  try {
    const { reason } = req.body;
    const record = await CompanyEmail.findByIdAndUpdate(req.params.emailId,
      { status: 'REVOKED', revocationReason: reason || 'Revoked by Super Admin',
        retentionExpiresAt: new Date(Date.now() + 30 * 86400000) }, { new: true });
    if (!record) return res.status(404).json({ message: 'Not found' });
