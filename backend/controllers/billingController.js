...
exports.createInvoice = async (req, res, next) => {
  try {
    ...
    const invoice = await Invoice.create({
      ...
      client:         clientId,
      lineItems:      items,
      subtotal, taxRate: rate, taxAmount,
      totalAmount:    total,
      balance:        total,
  ...
  try {
    ...
    if (invoice.status === 'PAID') {
      try {
        ...
        if (pts > 0) {
          await require('../models/CRMClient').findByIdAndUpdate(
            invoice.clientId, { $inc: { loyaltyPoints: pts } }
          );
        }
      } catch(_) {}
    }
    ...
    try {
      ...
      emitPaymentResult(checkId, {
        success: invoice.status === 'PAID', invoiceId: invoice._id,
        mpesaRef: invoice.mpesaRef, amount: invoice.amountPaid, paidAt: invoice.paidAt,
      });
    } catch (_) {}
